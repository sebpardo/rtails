...
sample_bias_corr <- function(d) {
  d_exp <- exp(d)
  if (any(d_exp == Inf)) stop("Mean cannot be estimated for bias correction as draws are too heavy-tailed: at least one draw approximates 'Inf' when exponentiated. Please adjust distribution parameters accordingly.")
