...
for situations where it is thought there are 2 discrete state that have different
...
(*Scomber scombrus*) shows years or very high recruitment once or twice per decade.
...
Pacific Cod in Alaska is a population where extreme low recruitment is 
hypothesized to have been caused by a lack of spawning environment due to 
elevated bottom temperatures. It's probably not as extreme and the concern 
is about downward only events, and seem to cycle more on a decadal time span.
For this population, a negatively-skewed mixture normal would be suitable
as it can incorporate both extreme low recruitment (skewness of wider distribution)
and it's decadal nature (`rate = 1/10`):
...
The main usage of this package is to provide random recruitment deviations 
to be used for Management Strategy Evaluation (MSE) using the `SAMtool` package.
This can be done directly by providing recruitment deviates as a custom parameter,
especifically the `Perr_y` parameter:


```{r samtool-validcpars}
library(DT)
library(SAMtool)
MSEtool:::cpars_info %>% dplyr::filter(Var == "Perr_y") %>% datatable()
```
